...
def file(type_id):
    ...
    elif current_user.school.train_status is 1 and not current_user.can('train_manage') and \
            not TrainStudent.is_train_student():
...
@login_required
def team():
    teams = TrainTeam.get_teams()
    return render_template('train/team.html', active_flg=['train', 'team'], teams=teams)
...
def team_member_edit(team_id):
    if request.method == 'POST':
        ...
        if not member1 or not member2 or not member3:
            ...
            TrainStudent.set_team(member1, team_id)
            TrainStudent.set_team(member2, team_id)
            TrainStudent.set_team(member3, team_id)
            return redirect(url_for('train.team'))
    # the_team = TrainTeam.query.get_or_404(int(team_id))
    select_student = TrainStudent.get_select()
    return render_template('train/team_member_edit.html', active_flg=['train', 'team'],
...
def grade_manage(team_id):
    ...
    return render_template('train/grade_manage.html', active_flg=['train', 'team'],
